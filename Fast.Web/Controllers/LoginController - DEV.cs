	...
	public class LoginController : Controller
	{
			...
			if (Session["UserLogon"] == null)
			{
				...
				if (usePageAuth)
				{
					...
					try
					{
						...
						{
							string[] userkuLists = { "pmi\\isadamm", "pmi\\rtama", "pmi\\eperdana" };
							if (userkuLists.Contains(userku.ToLower()))
		...
		public ActionResult SignIn(UserModel model)
		{
			...
			try
			{
				...
				{
					string[] userkuLists = { "pmi\\isadamm", "pmi\\rtama", "pmi\\eperdana" };
					if (userkuLists.Contains(model.UserName.ToLower()))
						return RedirectToAction("Index", "Home");
					else
						return RedirectToAction("Index", "Unauthorized");
		...
		private bool ValidateUsername(string username, out long userId, out string errorMessage)
		{
			...
			else
			{
				...
				string[] userkuLists = { "pmi\\isadamm", "pmi\\rtama", "pmi\\eperdana" };
				if (userkuLists.Contains(username.ToLower()))
				{
