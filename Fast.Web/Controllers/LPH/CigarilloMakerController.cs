...
{
	[CustomAuthorize("cigarillomaker")]
	public class CigarilloMakerController : BaseController<LPHModel>
	{
		#region ::Services::
		...
		#region ::Constructor::
		public CigarilloMakerController(
		...
		[HttpPost]
		public ActionResult Create(List<LPHExtrasModel> detailExtras, LPHSubmissionsModel detailSubmission, List<LPHValuesModel> detailValue, List<LPHComponentsModel> detailComponent)
		{
			try
			{
		...
		public ActionResult CheckPrev(long lphid = 0, string machine = "")
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _lphSubmissionsAppService.FindBy("LPHHeader", "CigarilloMakerController", true); //ambil yang sudah submit saja
		...
		public ActionResult Submitted(long lphid, int type = 0)
		{
			...
			ViewBag.PageHeader = "LPH Cigarillo Maker";
			if (type == 1)
				ViewBag.PageHeader = "LPH Cigarillo Maker <span class='tx-success'>(Previous Submit)</span>";
