...
{
    [CustomAuthorize("gwgeneral")]
    public class GWGeneralController : BaseController<LPHModel>
    {
        #region ::Services::
        ...
        public ActionResult Index(bool reset = false)
        {
            ...
            ViewBag.AccountProdCenterID = AccountProdCenterID;
            ...
            ViewBag.Maker = LPHSPHelper.GetMachineList(_machineAppService, locationIdList, model.Machines, true, false);
            ViewBag.Packer = LPHSPHelper.GetMachineList(_machineAppService, locationIdList, model.Machines, false, true);
            ...
            List<BrandModel> brandListFA = brandList.Where(x => locationIdList.Any(y => y == x.LocationID) && x.Code[0] == 'F').OrderBy(x => x.Code).ToList();
            ...
            ViewBag.BrandCodeFA = brandModelListFA;

            string referenceString = _referenceAppService.GetBy("Name", "ResourceCigarette");
            ReferenceModel referenceModel = referenceString.DeserializeToReference();
            ViewBag.ResourceCigarette = null;

            if (referenceModel != null)
            {
                string referenceDetailString = _referenceDetailAppService.FindBy("ReferenceID", referenceModel.ID);
                ViewBag.ResourceCigarette = referenceDetailString.DeserializeToRefDetailList();
            }
        ...
        public ActionResult Edit(long lphid)
        {
            ...
            if (lphModel != null && lphModel.Header != null)
            {
                ...
                List<long> locationIdList = _locationAppService.GetLocIDListByLocType(AccountProdCenterID, "productioncenter");
                ViewBag.Maker = LPHSPHelper.GetMachineList(_machineAppService, locationIdList, model2.Machines, true, false);
                ViewBag.Packer = LPHSPHelper.GetMachineList(_machineAppService, locationIdList, model2.Machines, false, true);
        ...
        public ActionResult CheckPrev(long lphid = 0)
        {
            try
            {
                if (lphid != 0)
                {
                    string submissions = _lphSubmissionsAppService.FindBy("LPHHeader", "GWGeneralController", true); //ambil yang sudah submit saja
        ...
        public ActionResult Submitted(long lphid, int type = 0)
        {
            ...
            ViewBag.PageHeader = "LPH GW General";
            if (type == 1)
                ViewBag.PageHeader = "LPH GW General <span class='tx-success'>(Previous Submit)</span>";
            ...
            //string wppString = _wppAppService.FindBy("Date", DateTime.Now.ToString("yyyy-MM-dd"));
            //ViewBag.WPP = wppString.DeserializeToWppList();
        ...
        public ActionResult PrintBarcode(string barcode, string info, string desc)
        {
        ...
        public ActionResult GetBrand(string brand)
        {
            try
            {
                string machineString = _brandAppService.GetBy("Code", brand);
                BrandModel machineModel = machineString.DeserializeToBrand();

                if (machineModel != null && machineModel.Description != null)
                {
                    return Json(new { Status = true, desc = machineModel.Description }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { Status = true, desc = "" }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
            {
