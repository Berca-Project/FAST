...
{
	[CustomAuthorize("primarycresdryingpacking")]
	public class LPHPrimaryCresDryingPackingController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryCresDryingPackingController(
		...
		// GET: LPHPrimaryCresDryingPackingController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryCresDryingPackingController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [PSS4_Runtime].[dbo].[NpssReportBatchData] " +
					...
					"((EventType = 'END L2100P' and DataName in ('Blend','Final-OV-Average','Dryer-InletOV-Average','Dryer-WB-Qty-Real','STS_STEAM_FLOWRATE','3Dryer-TobFlow-Average')) or" +
					"(EventType = 'END Line 2100' and DataName in ('SiloDestination','I2076 flowSP','FBD_ZONE_1_TEMPERATURE','FBD_ZONE_2_TEMPERATURE','FBD_ZONE_3_TEMPERATURE')) or" +
					"(EventType = 'ST Line 2100' and DataName in ('Batch ID')))";
                ...
                if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
                                    (EventType = 'ST STEM CutDry' and DataName in ('Blend Code','ST_CDS_destinationSiloDi','ST_CDS_flowSP','ST_CDS_STSSteamFlowRate','ST_CDS_outMoistSP'))
	                                or (EventType = 'END Stem CutDry' and DataName in ('END_CDS_RevLev','END_CDS_avginMoist','2ndBatchID','END_CDS_weigherTotalizer','END_CDS_avgAirTemp','END_CDS_avgSteamPss','END_CDS_avgTempOut','END_CDS_weigherTotalizer'))
	                                or (EventType = 'CRES END Drying' and DataName in ('END_CDS_STS_Inlet_Tobacco'))
        ...
        // GET: LPHPrimaryCresDryingPackingController/Details/5
        public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryCresDryingPackingController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryCresDryingPackingController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryCresDryingPackingController/Edit/5
		...
		// POST: LPHPrimaryCresDryingPackingController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryCresDryingPackingController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryCresDryingPackingController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryCresDryingPackingController", true); //ambil yang sudah submit saja
