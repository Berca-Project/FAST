...
{
	[CustomAuthorize("primarywhitelinepackingwhite")]
	public class LPHPrimaryWhiteLinePackingWhiteController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryWhiteLinePackingWhiteController(
		...
		// GET: LPHPrimaryWhiteLinePackingWhiteController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryWhiteLinePackingWhiteController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//        " DataName in ('Blend','BatchID','SAP Production Order'," +
					//        " 'CartonDataQty','CartonWeighSp','CartonDataTobNetWeig')"+
					//        " and EventType = 'Packing L1'"+
					...
					queryAle = @"SELECT [EventDateTimeLocal],CONCAT([Category],' ',[EventType]) as EventType,[DataName],[Value] FROM [NpssReportBatchData]
                        WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and (
                             ((EventType='Packing L1' or EventType='Packing L2') and DataName in ('Blend','BatchID','SAP Production Order',
	                        'CartonDataQty','CartonWeighSp','CartonDataTobNetWeig')) or DataName in ('Report.CFSiloLoadSel')
		...
		[HttpPost]
		public ActionResult getPSSDataCLU(String BatchID)
		{
			try
			{
				BatchID = BatchID.Trim();
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//        " WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent = '" + BatchID + "') and"+
					//        " DataName in ('Blend','BatchID','SAP Production Order'," +
					//        " 'JacobiFeeder[1].TotalTobacco','JacobiFeeder[2].TotalTobacco','Batch Size') "+
					//        " and EventType = 'Packing L2'"+
					//        " ORDER BY EventType DESC";
					queryAle = @"SELECT [EventDateTimeLocal],CONCAT([Category],' ',[EventType]) as EventType,[DataName],[Value] FROM [NpssReportBatchData]
                            WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                            (EventType='Packing L1' or EventType='Packing L2') and DataName in ('Blend','BatchID','SAP Production Order',
	                            'JacobiFeeder[1].TotalTobacco','JacobiFeeder[2].TotalTobacco','TotalTobacco','Batch Size')
                            )ORDER BY EventDateTimeLocal ASC";
					compoAndSAPDset = new DataSet();
					using (SqlConnection con = new SqlConnection(strConString))
					{
		...
		// GET: LPHPrimaryWhiteLinePackingWhiteController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryWhiteLinePackingWhiteController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryWhiteLinePackingWhiteController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryWhiteLinePackingWhiteController/Edit/5
		...
		// POST: LPHPrimaryWhiteLinePackingWhiteController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryWhiteLinePackingWhiteController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryWhiteLinePackingWhiteController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryWhiteLinePackingWhiteController", true); //ambil yang sudah submit saja
