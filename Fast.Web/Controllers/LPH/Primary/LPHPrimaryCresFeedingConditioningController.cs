...
{
	[CustomAuthorize("primarycresfeedingconditioning")]
	public class LPHPrimaryCresFeedingConditioningController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryCresFeedingConditioningController(
		...
		// GET: LPHPrimaryCresFeedingConditioningController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryCresFeedingConditioningController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [PSS4_Runtime].[dbo].[NpssReportBatchData] " +
					...
					" ((EventType = 'END L2000P' and DataName in ('3DCC-Stem-TobFlow-Average','Infeed ExitOVAvg','Infeed WaterQty')) or" +
					" (EventType = 'END Line 2000' and DataName in ('Blend','SiloDestination','NomiMoistureIn','TobTotalizer','OutAvgTobTemp','TobTotalizerInSilo')) or" +
					" (EventType = 'ST Line 2000' and DataName in ('Batch ID')))";
                ...
                if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and (
	                                (EventType='ST Stem Infeed' and	DataName in ('Blend Code','BatchID','Numeric Sap Batch ID','Composition Version'))
	                                or (EventType='END Stem Infeed' and DataName in ('END_IST_weigherTotalizeroutSC','END_IST_WaterTotalizer'))
	                                or (EventType='CRES END Infeed' and DataName in ('2ndBatchID','CRES_End_Inf_Rev Level','CRES_Inf_Flowrate','CRES_Inf_InletOV_SP','CRES_Inf_ExitOV_Avg','CRES_Inf_ExitTempSP'))
        ...
        // GET: LPHPrimaryCresFeedingConditioningController/Details/5
        public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryCresFeedingConditioningController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryCresFeedingConditioningController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryCresFeedingConditioningController/Edit/5
		...
		// POST: LPHPrimaryCresFeedingConditioningController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryCresFeedingConditioningController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryCresFeedingConditioningController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryCresFeedingConditioningController", true); //ambil yang sudah submit saja
