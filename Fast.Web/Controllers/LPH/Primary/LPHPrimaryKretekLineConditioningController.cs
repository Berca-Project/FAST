...
{
	[CustomAuthorize("primarykreteklineconditioning")]
	public class LPHPrimaryKretekLineConditioningController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryKretekLineConditioningController(
		...
		// GET: LPHPrimaryKretekLineConditioningController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryKretekLineConditioningController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
                    ...
                    (EventType = 'END IKR E' and DataName in ('Blend','Silo Destionation','Tob Totalizer in Silo','Krosok FC Tob Qty Totalizer')) or
                    (EventType = 'END IKR P' and DataName in ('V4-KRS-DCC-FC-Water-QtyReal','V4-KRS-DCC-FC-ExitOV-DryReal','V4-KRS-DCC-FC-AirTemp-Avg','V4-KRS-DCC-FC-TobTemp-Avg','V4-KRS-DCC-FC-ExitOV-Avg','V4-KRS-DCC-BC-AppRatPV-Avg','V4-KRS-DCC-BC-Solution-QtyReal')) or
                    (EventType = 'ST IKR E' and DataName in ('Batch ID')))";
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					// -- Silo Total Blend,Totalizer Casing krosok, Casing rate, Airtemp1, Airtemp2
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
                                    (EventType ='Lamina END CKR' and DataName in('END_CsgKR_BSCylShellTemp','END_CsgKR_KR_BulkSiloLoadSel','END_CsgKR_WB250045TobFlowAvg','END_CsgKR_WB250045TotalDry','END_CsgKR_TM250045Avg','Total Casing Krosok','END_CsgKR_TobTemp250070Avg','END_CsgKR_WB250045TotalWet','V3_END_CsgKR_Cyl250070SPAvg','END_CsgKR_TobTemp250070Avg'))
	                               or (EventType ='END Infeed KR' and DataName in('END_IKR_KR_PBSiloLoadSel','END_IKR_DCC_2_60TobTempAvg201','END_IKR_DCC_2_60TotalWtr201','END_IKR_DCC_2_60TobTempAvg202','END_IKR_DCC_2_60TotalWtr202'))
	                                or (EventType ='Lamina END IRJ' and DataName in('END_IRJ_MM250128Avg101','END_IRJ_WB100045BTotWetTob','END_IRJ_WB100045BTotDryTob'))
	                                or (EventType ='ST Infeed KR' and DataName in('Batch ID','Blend Code'))
	                                or (EventType ='END Infeed KR' and DataName in('Batch ID'))
	                                or (EventType ='ST Casing KR' and DataName in('Batch ID'))
	                                or (EventType ='END Casing KR' and DataName in('Batch ID'))
									or (EventType ='Lamina END IKR' and dataname in ('END_IKR_WB200045BTotWetTob','END_IKR_WB200045BTotDryTob','END_IKR_KR_TBSiloLoadSel','END_CsgKR_Cyl250070TotCasing','END_CsgKR_BSApplRate','END_IKR_DCC_2_60AirTempAvg201','END_IKR_DCC_2_60AirTempAvg202'))
                                )ORDER BY EventDateTimeLocal ASC";
					compoAndSAPDset = new DataSet();
					using (SqlConnection con = new SqlConnection(strConString))
					{
						SqlCommand cmd = new SqlCommand(@"SELECT TOP 1 [EngineeringChangeNumber],PV.[ParamValue]
                          FROM [PSS4].[dbo].[NpssBatch] B
                          INNER JOIN [PSS4_Runtime].[dbo].[NpssParamValue] PV ON B.[BatchID] = PV.[ContextKey]
                          WHERE [BatchIdent] = '" + BatchID + @"' AND PV.[ParamID] = '17723'", con);
		...
		[HttpPost]
		public ActionResult getPSSDataRajangan(String BatchID)
		{
			try
			{
				BatchID = BatchID.Trim();
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                    WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent = '" + BatchID + @"' and starttimelocal >= starttimelocal - 100) and
                    ((EventType = 'END IKR P' and DataName in ('SAP Solution Krosok Batch id 1 Str')) or
                    (EventType = 'END IRJ E' and DataName in ('Blend','Silo Destination','01. Rajangan Total Actual weight','TobTotalizerInSilo')) or
                    (EventType = 'END IRJ P' and DataName in ('Blend','BS-WB-Dry-Real','V4-RAJ-DCC-RJ-Water-QtyReal','V4-RAJ-DCC-RJ-ExitOV-Avg','V4-RAJ-BS-AppRatPV-Avg','V4-RAJ-BS-Solution-QtyReal','V4-RAJ-DCC-RJ-TobTemp-Avg','V4-RAJ-DCC-RJ-AirTemp-Avg')) or
                    (EventType = 'ST IRJ E' and DataName in ('Batch ID')))";
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//    " DataName in('Blend Code','Numeric Sap Batch ID','END_CsgRJ_CasingBatchID','END_IRJ_RJ_PBSiloLoadSel','END_CsgRJ_RJ_BulkSiloLoadSel',"+
					//    " 'END_IRJ_WB100045BTotWetTob','END_IRJ_WB100045BTotDryTob','DCC_1_60AirTempAvg101','DCC_1_60AirTempAvg102','END_IRJ_DCC_1_60TotalWtr101','END_IRJ_DCC_1_60TotalWtr102',"+
					//    " 'END_CsgRJ_WB250045TotalWet','END_CsgRJ_WB150045TotalDry','END_CsgRJ_TM150050','END_CsgRJ_BSCylShellTemp','END_CsgRJ_WB150045Total','END_CsgRJ_TobTemp150070','V3_END_CsgRJ_Cyl150070SPAvg','END_CsgRJ_TobTemp150070','Total Casing Rajangan')"+
					//    " ORDER BY EventType DESC";
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                        WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100) and
	                        ((EventType='Lamina END CRJ' and DataName in('Blend Code','END_CsgRJ_BSCylShellTemp','END_CsgRJ_WB150045Total','END_CsgRJ_WB150045TotalDry','END_CsgRJ_TM150050','END_CsgRJ_TobTemp150070','END_CsgRJ_RJ_BulkSiloLoadSel','END_CsgRJ_CasingBatchID','V3_END_CsgRJ_Cyl150070SPAvg','Total Casing Rajangan'))
	                        or (EventType='Lamina END IRJ' and DataName in('Blend Code','END_IRJ_DCC_1_60TotalWtr101','END_IRJ_DCC_1_60TotalWtr102','END_IRJ_RJ_PBSiloLoadSel','END_IRJ_WB100045BTotWetTob','END_IRJ_WB100045BTotDryTob','END_IRJ_MM100080XTobTempAvg101','END_IRJ_MM100080XTobTempAvg102','END_IRJ_MM250128Avg101','END_IRJ_MM250128Avg102','END_CsgRJ_BSApplRate','End_CsgRJ_Cyl150070TotCasing'))
	                        or (EventType='END Infeed RJ' and DataName in('DCC_1_60AirTempAvg101','DCC_1_60AirTempAvg102'))
	                        or (EventType ='ST Infeed RJ' and DataName in('Batch ID'))
	                        or (EventType ='ST Casing RJ' and DataName in('Batch ID'))
                            or (EventType ='Lamina END IKR' and dataname in ('END_IKR_KR_TBSiloLoadSel'))
	                        or (EventType ='END Casing RJ' and DataName in('Batch ID'))
	                        or (DataName in('Numeric Sap Batch ID','END_CsgRJ_WB250045TotalWet'))
                        )ORDER BY EventDateTimeLocal ASC";
					compoAndSAPDset = new DataSet();
					using (SqlConnection con = new SqlConnection(strConString))
					{
		...
		public string DataTableToJSONWithJavaScriptSerializer(DataTable table, DataTable compoAndSAP, DataTable compoBlend)
		{
			...
			if (compoAndSAP != null && compoAndSAP.Rows.Count > 0) foreach (DataColumn col in compoAndSAP.Columns)
				{
					childRow = new Dictionary<string, object>
				{
					{ "EventType" , ""},
			...
			if (compoBlend != null && compoBlend.Rows.Count > 0) foreach (DataColumn col in compoBlend.Columns)
				{
					childRow = new Dictionary<string, object>
				{
					{ "EventType" , ""},
		...
		// GET: LPHPrimaryKretekLineConditioningController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryKretekLineConditioningController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineConditioningController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineConditioningController/Edit/5
		...
		// POST: LPHPrimaryKretekLineConditioningController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineConditioningController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineConditioningController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryKretekLineConditioningController", true); //ambil yang sudah submit saja
