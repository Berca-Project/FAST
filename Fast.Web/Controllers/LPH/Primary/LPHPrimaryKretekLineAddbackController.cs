...
{
	[CustomAuthorize("primarykreteklineaddback")]
	public class LPHPrimaryKretekLineAddbackController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryKretekLineAddbackController(
		...
		// GET: LPHPrimaryKretekLineAddbackController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryKretekLineAddbackController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
                ...
                // Oos put out mapping : ,'04. Addback OFFSPEC Totalizer','SAP Addback OOSpec Batch id 1 Str'
                var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
                    ...
                    ((EventType = 'END IKR P' and DataName in ('SAP Solution Krosok Batch id 1 Str')) or
                    (EventType = 'END CUT E' and DataName in ('ComfinalSiloDestination')) or
                    (EventType = 'END DRY E' and DataName in ('Weigher 3250Totalizer','Final SiloDestination')) or
                    (EventType = 'END DRY P' and DataName in ('Master-WB-Dry-Real','Master-OV-Average','V4-Master-WB-FlowrateAvg')) or
                    (EventType = 'END ET/IS/SL' and DataName in ('AddBack-ET-Qty-Real','04. Addback RTC Totalizer''BLEND','SAP ET Batch id 01','04. Addback ET Totalizer','AddBack-ET-OV-Average','SAP CRES Batch id 01','04. Addback CRES Totalizer','AddBack-IS-OV-Average','SAP RTC Batch id 01','SAP RTC-Qty-Real-01','AddBack-SL-OV-Average')) or
                    (EventType = 'END ET/IS/SL P3' and DataName in ('ET Tob Flowrate Consistency_Avg','IS Tob Flowrate Consistency_Avg','SL Tob Flowrate Consistency_Avg')) or
                    (EventType = 'END FL CL P' and DataName in ('Addback SMALL Totalizer','04. Addback CLOVE Totalizer','05. Flavour Totalizer','Addback CSF Totalizer','AddBack-RS-Qty-Real','SAP Addback Clove Batch id 01','SAP Addback CSF Batch id 01','SAP Addback Short Batch id 01','SAP Solution Flavour Batch id 1 Str','V4-Clove-OV-Average','V4-CSF-OV-Average','V4-Final-OV-Avg','V4-Final-OV-StDev','V4-RS-OV-Average')) or
                    (EventType = 'ST DRY E' and DataName in ('Batch ID')))";

				DataSet compoAndSAPDset = null;
				DataSet BlendDset = null;
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//    " DataName in('Blend Code','Numeric Sap Batch ID'," +
					//    " 'MasterWB620.150TotWetTob','MasterWB620.150TotDryTob','MasterMM620.151Average','MasterWB620.150AverageTob'," +
					//    " 'ST_ABack_ProdCodeFlav','SLWB620.175TotWetTob','Total Flavor'," +
					//    " 'ST_ABack_ProdCodeCLs','END_ABack_WB600150AvgTob','Total CLOVES','END_AB_MM600150Average_Clove'," +
					//    " 'ST_ABack_ProdCodeCSF','END_ABack_WB600155AvgTob','Total CSF','END_AB_MM600155Average_CSF'," +
					//    " 'ST_ABack_ProdCodeET','END_ABack_WB600120AvgTob','Total ET','END_AB_MM600120Average_ET'," +
					//    " 'ST_ABack_ProdCodeOffSpec','END_ABack_WB600160AvgTob','Total OffSpec'," +
					//    " 'ST_ABack_ProdCodeCRES','END_ABack_WB600125AvgTob','Total CRES','END_AB_MM600125Average_CRES'," +
					//    " 'ST_ABack_ProdCodeRCS','END_ABack_WB600130AvgTob','RTC  Batch ID Unloaded 1','Total RTC','END_AB_MM600130Average_RCS'," +
					//    " 'END_ABack_Size','END_AB_MM600230Average_Final','V3 END_AB_MM600230StdDev_BFS')" +
					//    " ORDER BY EventType DESC";
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                        WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                        (EventType = 'ST AddBack' and DataName in ('Batch ID','ST_ABack_ProdCodeFlav','ST_ABack_ProdCodeCLs','ST_ABack_ProdCodeCSF','ST_ABack_ProdCodeET','ST_ABack_ProdCodeOffSpec','ST_ABack_ProdCodeCRES','ST_ABack_ProdCodeRCS'))
	                        or (EventType = 'Lamina END AB' and DataName in ('Total Flavor','Total ET','Total CRES','Total RTC','Total CLOVES','Total CSF','Total OffSpec','END_AB_MM600120Average_ET','END_AB_MM600125Average_CRES','END_AB_MM600130Average_RCS','END_AB_MM600150Average_Clove','END_AB_MM600155Average_CSF','END_AB_MM600230Average_Final','V3 END_AB_MM600230StdDev_BFS'))
	                        or (EventType = 'END AddBack' and DataName in ('Batch ID', 'Blend Code','END_ABack_WB600120AvgTob','END_ABack_WB600125AvgTob','END_ABack_WB600130AvgTob','END_ABack_WB600160AvgTob','END_ABack_WB600155AvgTob','END_ABack_WB600150AvgTob','END_ABack_Size'))
	                        or (EventType = 'Lamina END Addback Intermediet Usage P1' and DataName in ('RTC  Batch ID Unloaded 1'))
	                        or (DataName in('END_IKR_KR_TBSiloLoadSel','END_AB_WB600230AvgTob','END_AB_MM600230StdDev_Final','END_AB_CFSiloLoadSel','END_AB_WB600010TotWetTob_Mst','END_AB_WB600010TotDryTob_Mst','END_AB_MM600010Average_Mst','END_AB_WB600010AvgTob_Mst','END_AB_FlavorAverage','END_AB_FlavorBatchID','END_AB_CLV_BatchID_Silo1','END_AB_CSF_BatchID_Silo1','Diet Batch ID Unloaded 1','END_AB_MM600160Average_OS','END_AB_CRES_BatchID_Silo1'))
                        )ORDER BY EventDateTimeLocal ASC";
					compoAndSAPDset = new DataSet();
					using (SqlConnection con = new SqlConnection(strConString))
					{
						SqlCommand cmd = new SqlCommand(@"SELECT [EngineeringChangeNumber],PV.[ParamValue]
                            FROM [PSS4].[dbo].[NpssBatch] B
                            INNER JOIN [PSS4_Runtime].[dbo].[NpssParamValue] PV ON B.[BatchID] = PV.[ContextKey]
                            WHERE [BatchIdent] = '" + BatchID + @"' AND PV.[ParamID] in (
	                            SELECT [ParameterID]
	                            FROM [PSS4_Runtime].[dbo].[NpssParam]
	                            WHERE [ParameterName] = 'Numeric Sap Batch ID'
                            )", con);
						SqlDataAdapter da = new SqlDataAdapter(cmd);
						da.Fill(compoAndSAPDset);
					}

					BlendDset = new DataSet();
					using (SqlConnection con = new SqlConnection(strConString))
					{
						SqlCommand cmd = new SqlCommand(@"SELECT [BlendCode] as 'Blend Code' FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"'", con);
						SqlDataAdapter da = new SqlDataAdapter(cmd);
						da.Fill(BlendDset);
					}
				}
				...
				var data = DataTableToJSONWithJavaScriptSerializer(dset.Tables[0], compoAndSAPDset?.Tables[0], BlendDset?.Tables[0]);
		...
		public string DataTableToJSONWithJavaScriptSerializer(DataTable table, DataTable compoAndSAP, DataTable compoBlend)
		{
			...
			if (compoAndSAP != null && compoAndSAP.Rows.Count > 0) 
				foreach (DataColumn col in compoAndSAP.Columns)
				{
					childRow = new Dictionary<string, object>
					{
						{ "EventType" , ""},
						{ "DataName" , col.ColumnName },
						{ "Value" , compoAndSAP.Rows[0][col] },
						{ "EventDateTimeLocal" , ""},
					};
					parentRow.Add(childRow);
				}

			if (compoBlend != null && compoBlend.Rows.Count > 0) 
				foreach (DataColumn col in compoBlend.Columns)
				{
					childRow = new Dictionary<string, object>
					{
						{ "EventType" , ""},
						{ "DataName" , col.ColumnName },
						{ "Value" , compoBlend.Rows[0][col] },
						{ "EventDateTimeLocal" , ""},
					};
					parentRow.Add(childRow);
				}
		...
		// GET: LPHPrimaryKretekLineAddbackController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryKretekLineAddbackController/Create
		...
		// POST: LPHPrimaryKretekLineAddbackController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineAddbackController/Edit/5
		public ActionResult Edit(long lphid)
		{
			...
			int flag = rnd.Next(1000, 9000);
			ViewBag.Special = flag.ToString();
			LoadFlagPrimary(lphid, flag);

			return View("index", model);
		}

		// POST: LPHPrimaryKretekLineAddbackController/Edit/5
		[HttpPost]
		public async Task<ActionResult> Edit(long id, List<PPLPHExtrasModel> detailsExtras, List<PPLPHComponentsModel> detailComponent, List<PPLPHValuesModel> detailValue, int isSubmit = 0, int Special = 0)
		{
			try
			{
				await EditLPHPrimary(_userAppService, _employeeAppService, _ppLphSubmissionsAppService, _ppLphExtrasAppService, _ppLphComponentsAppService, _ppLphValuesAppService, _ppLphValueHistoriesAppService, _ppLphApprovalAppService, id, detailsExtras, detailValue, isSubmit, Special);

				PPLPHSubmissionsModel submitModel = _ppLphSubmissionsAppService.GetByNoTracking("LPHID", id).DeserializeToPPLPHSubmissions();
				//if (submitModel != null && submitModel.Flag != Special)
				//{
		...
		// GET: LPHPrimaryKretekLineAddbackController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineAddbackController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryKretekLineAddbackController", true); //ambil yang sudah submit saja
