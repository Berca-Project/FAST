...
{
	[CustomAuthorize("primarycsfinfeedconditioning")]
	public class LPHPrimaryCSFInfeedConditioningController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryCSFInfeedConditioningController(
        ...
        private void GetReferenceTarget()
        {
            ...
            string getIDReference = _referenceAppService.FindBy("Name", "TargetFCSF", true);

            if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
            {
                getIDReference = _referenceAppService.FindBy("Name", "WestTargetFCSF", true);
        ...
        // GET: LPHPrimaryCSFInfeedConditioningController
        public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryCSFInfeedConditioningController"));
		...
		[HttpPost]
		public ActionResult getPSSDataInfeed(String BatchID)
		{
			try
			{
				BatchID = BatchID.Trim();
				string strConString = ConfigurationManager.ConnectionStrings["AdoPSS4Conn"].ConnectionString;
				string queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] " +
								...
								" ((EventType = 'ST infClove' and DataName in ('Batch ID')) or " +
                                " (EventType = 'ST C_Cleaning' and DataName in ('Blend','DestinationSilo')) or" +
								" (EventType = 'END infClove' and DataName in ('TotBatchWeight','I40120 matSeparated')))";
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                                (EventType = 'END CSF Zone2' and DataName in ('Blend Code','CSF Z2 Silo Destination'))
	                                or (EventType = 'ST CSF Zone2' and DataName in ('Batch ID'))
	                                or (EventType = 'ST Inf_CL' and DataName in ('Batch ID'))
				...
				using (SqlConnection con = new SqlConnection(strConString))
				{
					...
					data = DataTableToJSONWithJavaScriptSerializer(dset.Tables[0]);
				}
		...
		public ActionResult getPSSDataConditioning(String BatchID)
		{
			try
			{
				...
				string queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]" +
								...
								" (EventType = 'END C_CleanP' and DataName in ('I1140 SteamPressAVG','I1160 SteamPV','DCC1CloveTempAvg','DCC2CloveTempAvg','DCC1FCTobFlowAverage','DCC2FCTobFlowAverage','DCC1FCWaterQtyReal','DCC2FCWaterQtyReal')) or" +
								" (EventType = 'END C_Cleaning' and DataName in ('DestinationSIlo','Weigher Totalizer5004','Weigher Totalizer5003'))) ";
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					strConString = ConfigurationManager.ConnectionStrings["AdoPSS4Conn2"].ConnectionString;
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                                ...
	                                or (EventType = 'END CL_PrebSilo' and DataName in ('ClovePreblendSilo_43030_flowSP','ClovePreblendSilo_43031_flowSP','ClovePreblendSilo_1_WaterApplied','ClovePreblendSilo_2_WaterApplied','ClovePreblendSilo_1_TotalWetQty','ClovePreblendSilo_2_TotalWetQty'))
		...
		// GET: LPHPrimaryCSFInfeedConditioningController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryCSFInfeedConditioningController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryCSFInfeedConditioningController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryCSFInfeedConditioningController/Edit/5
		...
		// POST: LPHPrimaryCSFInfeedConditioningController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryCSFInfeedConditioningController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryCSFInfeedConditioningController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryCSFInfeedConditioningController", true); //ambil yang sudah submit saja
