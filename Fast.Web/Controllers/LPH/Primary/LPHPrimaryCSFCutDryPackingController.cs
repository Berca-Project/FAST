...
{
	[CustomAuthorize("primarycsfcutdrypacking")]
	public class LPHPrimaryCSFCutDryPackingController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryCSFCutDryPackingController(
		...
		// GET: LPHPrimaryCSFCutDryPackingController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryCSFCutDryPackingController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
                    ...
                    ((EventType = 'ST Clove Flake Zone1' and DataName in ('Batch ID','Blend','silo Destination')) or
                    (EventType = 'ST Clove Flake Zone2' and DataName in ('Batch ID','I60102tobQtySpRe','I60202tobQtySpRe','I60302tobQtySpRe')) or
                    (EventType = 'ST Clove Flake Zone3' and DataName in ('Batch ID')) or
                    (EventType = 'END InfClove' and DataName in ('WeigherTotalizer')) or
                    (EventType = 'END Clove Flake Zone1' and DataName in ('Batch ID')) or
                    (EventType = 'END Clove Flake Zone 2' and DataName in ('Batch ID')) or
                    (EventType = 'END Clove Flake Zone 3' and DataName in ('I60132totalizerRe','I60232totalizerRe','I60130totalizerRe','I60230totalizerRe','I60130estMoistRe','I60230estMoistRe','I60020moistAvgRe')) or 
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                                (EventType = 'END CSF Zone2' and DataName in ('Blend Code'))
	                                or (EventType = 'ST CSF Zone2' and DataName in ('Batch ID'))
	                                or (EventType = 'ST CSF Zone4' and DataName in ('CSF Z4 Silo Destination'))
	                                or (EventType = 'END Inf_CL' and DataName in ('2ndBatchID'))
	                                or (EventType = 'CSF 2020' and DataName in ('MasterWB60130.TotWetTob','FlavCyl60132.TotFlavQty','FinalMM60021.Average','WB60102.TotWetTobQty','MM60109ExitOV.Average','WB60202.TotWetTobQty','MM60209ExitOV.Average','WB60302.TotWetTobQty','MM60310ExitOV.Average'))
                                )ORDER BY EventDateTimeLocal DESC";
				}
		...
		// GET: LPHPrimaryCSFCutDryPackingController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryCSFCutDryPackingController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryCSFCutDryPackingController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryCSFCutDryPackingController/Edit/5
		...
		// POST: LPHPrimaryCSFCutDryPackingController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryCSFCutDryPackingController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryCSFCutDryPackingController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryCSFCutDryPackingController", true); //ambil yang sudah submit saja
