...
{
	[CustomAuthorize("primarykreteklinefeeding")]
	public class LPHPrimaryKretekLineFeedingController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryKretekLineFeedingController(
		...
		// GET: LPHPrimaryKretekLineFeedingController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			string blend = _referenceDetailAppService.FindByNoTracking("ReferenceID", "14", true);
			List<ReferenceDetailModel> blendList = blend.DeserializeToRefDetailList();
			ViewBag.Blend = blendList;
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryKretekLineFeedingController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
									...
									" ((EventType = 'End IKR E' and DataName in ('BLEND','SILO Destionation','Krosok FC Tob Qty Totalizer')) or" +
									" (EventType = 'END IKR P' and DataName in ('Batch ID','SAP Solution Krosok Batch id 1 Str')) or" +
									" (EventType = 'END IRJ E' and DataName in ('SILO Destination','01. Rajangan Total Actual Weight')) or" +
									" (EventType = 'END IRJ P' and DataName in ('Batch ID')) or" +
									" (EventType = 'ST IKR E' and DataName in ('Batch ID')) or" +
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//                " DataName in('Blend Code','Numeric Sap Batch ID','Composition Version'," +
					//                " 'WB100045BTotWetTob','END_IKR_WB200045BTotWetTob')" +
					//                " ORDER BY EventType DESC";
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                                (EventType='END Infeed RJ' and DataName in('Blend Code','Numeric Sap Batch ID','Composition Version','WB100045BTotWetTob')
	                                or(EventType='END Infeed KR' and DataName in('END_IKR_WB200045BTotWetTob','END_IKR_KR_TBSiloLoadSel'))
	                                or(EventType='ST Infeed RJ' and DataName in('Batch ID'))
	                                or(EventType='ST Infeed KR' and DataName in('Batch ID'))
	                                or(DataName in('Numeric Sap Batch ID','Composition Version')))
		...
		// GET: LPHPrimaryKretekLineFeedingController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryKretekLineFeedingController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineFeedingController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineFeedingController/Edit/5
		public ActionResult Edit(long lphid)
		{
			#region Load Header
			string blend = _referenceDetailAppService.FindByNoTracking("ReferenceID", "14", true);
			List<ReferenceDetailModel> blendList = string.IsNullOrEmpty(blend) ? new List<ReferenceDetailModel>() : JsonConvert.DeserializeObject<List<ReferenceDetailModel>>(blend);
			ViewBag.Blend = blendList;
		...
		// POST: LPHPrimaryKretekLineFeedingController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineFeedingController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineFeedingController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryKretekLineFeedingController", true); //ambil yang sudah submit saja
