...
{
	[CustomAuthorize("primarykreteklinecuttingdrying")]
	public class LPHPrimaryKretekLineCuttingDryingController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryKretekLineCuttingDryingController(
		...
		// GET: LPHPrimaryKretekLineCuttingDryingController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryKretekLineCuttingDryingController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
                    ...
                    (EventType = 'END CUT E' and DataName in ('03. Combine Total Tobacco [raj + krs]','I2430metalDetecCount','I2400metalDetecCount','Blend','AvgCutWidth','Sampling1_CutWidth','Sampling2_CutWidth','Sampling3_CutWidth','Sampling4_CutWidth','Sampling5_CutWidth')) or
                    (EventType = 'END CUT P' and DataName in ('Cutter-A-MouthPressure-Average','Cutter-B-MouthPressure-Average','Cutter Break Time')) 
                    or (EventType = 'ST CUT E' and DataName in ('Batch ID')))";
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                        WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and
	                        DataName in('Blend Code','Numeric Sap Batch ID','END_Cut_MD300055MetalsCount','END_Cut_MD300075MetalsCount','BlendCert_CutTotTobKR','BlendCert_CutTotTobRJ','END_Cut_WB300240Ratio','END_Cut_BreakdownCount')
		...
		[HttpPost]
		public ActionResult getPSSDataDrying(String BatchID)
		{
			try
			{
				BatchID = BatchID.Trim();
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
                    ...
                    (EventType = 'END DRY E' and DataName in ('Final SiloDestination')) or
                    (EventType = 'END DRY FTD' and DataName in ('FTD-GasTemp-Average','FTD-GasFlow-Average','45110VelocitypneumaticAvg','Blend','Batch ID','FTD-RunTime-Real','FTD-WB-Qty-Real','FTD-InletOV-Average','FTD-ExitOV-Average')) or
                    (EventType = 'StartDryCal' and DataName in ('Batch ID')) or 
                    (EventType = 'StopDryCal' and DataName in ('Batch ID')) or 
                    (EventType = 'END Mixing Cylinder' and DataName in ('05. Final Weigher Total'))) ";
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//    " 'BlendCert_InletFTDTotTob',"+
					//    " 'BlendCert_InletFTDMC','END_FTDgasTemperatureAvg','END_FTDgasTemperatureAvg',"+
					//    " 'END_FTDCoolingFanSpeedSPRe','END_FTDoutletMoistAvg')"+
					//    " ORDER BY EventType DESC";
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                        WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
	                        (EventType ='Lamina END FTD' and DataName in('BlendCert_InletFTDTotTob','BlendCert_InletFTDMC','END_FTDgasTemperatureAvg','END_FTDCoolingFanSpeedSPRe','END_FTDoutletMoistAvg'))
	                        or (EventType ='Lamina END Cut' and DataName in('Blend Code','BlendCert_CutTotTobKR','BlendCert_CutTotTobRJ','END_Cut_WB300240Ratio'))
	                        or (EventType ='Lamina ST FTD' and DataName in('Batch ID'))
	                        or (EventType ='Lamina END FTD' and DataName in('Batch ID'))
	                        or (DataName in ('Numeric Sap Batch ID','END_FTDgasFlowAvg','END_IKR_KR_TBSiloLoadSel','END_AB_CFSiloLoadSel'))
		...
		// GET: LPHPrimaryKretekLineCuttingDryingController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryKretekLineCuttingDryingController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineCuttingDryingController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineCuttingDryingController/Edit/5
		...
		// POST: LPHPrimaryKretekLineCuttingDryingController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLineCuttingDryingController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryKretekLineCuttingDryingController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryKretekLineCuttingDryingController", true); //ambil yang sudah submit saja
