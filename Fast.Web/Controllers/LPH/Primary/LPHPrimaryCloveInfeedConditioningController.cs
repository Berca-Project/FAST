...
{
	[CustomAuthorize("primarycloveinfeedconditioning")]
	public class LPHPrimaryCloveInfeedConditioningController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryCloveInfeedConditioningController(
		...
		// GET: LPHPrimaryCloveInfeedConditioningController
		private void GetReferenceTarget()
		{
			...
			string getIDReference = _referenceAppService.FindBy("Name", "TargetFC", true);

            if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
            {
                getIDReference = _referenceAppService.FindBy("Name", "WestTargetFC", true);
			...
			if (referenceList.Count > 0)
			{
				...
				foreach (var value in cloveConList)
				{
					switch (value.Code)
					{
						case "T1":
		...
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryCloveInfeedConditioningController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] " +
								...
								" ((EventType = 'ST infClove' and DataName in ('DestinationSilo','Batch ID')) or" +
								" (EventType = 'END infClove' and DataName in ('WeigherTotalizer')) or" +
								" (EventType = 'ST C_Cleaning' and DataName in ('Blend')))";
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and (
                                    (EventType = 'ST Inf_CL' and DataName in ('Blend Code'))
	                                or (EventType = 'ST CL_ PrebSilo' and DataName in ('Batch ID')) 
	                                or (EventType = 'END Inf_CL' and DataName in ('2ndBatchID','InfeedClove_RevLev','InfeedClove_destinationSilo','InfeedClove_weigherTotalizer'))
									or (EventType = 'END CL_PrebSilo' and DataName in ('BR - Report.DCCC1.TobTemp.Average','BR - Report.DCCC2.TobTemp.Average'))
		...
		// POST: LPHPrimaryCloveInfeedConditioningController/getPSSData

		[HttpPost]
		public ActionResult getPSSDataConditioning(String BatchID)
		{
			try
			{
				BatchID = BatchID.Trim();
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] " +
							" WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent = '" + BatchID + "' and starttimelocal >= starttimelocal - 100) and " +
							" ((EventType = 'ST C_Cleaning' and DataName in ('Blend','Batch ID')) or " +
							" (EventType = 'END C_Cleaning' and DataName in ('Weigher Totalizer5004','Weigher Totalizer5003','I1160 avgSteamPress','I1140 avgSteamPress')) or " +
							" (EventType = 'END C_CleanP' and DataName in ('DCC1CloveTempAvg','DCC2CloveTempAvg','DCC1FCWaterQtyReal','DCC2FCWaterQtyReal','I1160CasingTotalizer','I1140CasingTotalizer'))) ";
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					strConString = ConfigurationManager.ConnectionStrings["AdoPSS4Conn2"].ConnectionString;
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                                WHERE EventID = ( SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent ='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and (
	                                (EventType = 'ST CL_ PrebSilo' and DataName in ('Blend Code','Batch ID')) 
	                                or (EventType = 'END CL_PrebSilo' and DataName in ('ClovePreblendSilo_destinationSilo','ClovePreblendSilo_43030_flowSP','V3_Infeed1-FC-Qty-Real','V3_Infeed2-FC-Qty-Real','ClovePreblend 43040_CaseCloveTotQty','ClovePreblend 43045_CaseCloveTotQty','ClovePreblendSilo_1_WaterApplied','ClovePreblendSilo_2_WaterApplied','BR - Report.DCCC1.TobTemp.Average','BR - Report.DCCC2.TobTemp.Average'))
	                                or (EventType = 'END Inf_CL' and DataName in ('2ndBatchID','InfeedClove_RevLev'))
                                )ORDER BY EventDateTimeLocal DESC";
				}
		...
		// GET: LPHPrimaryCloveInfeedConditioningController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryCloveInfeedConditioningController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryCloveInfeedConditioningController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryCloveInfeedConditioningController/Edit/5
		...
		// POST: LPHPrimaryCloveInfeedConditioningController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryCloveInfeedConditioningController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryCloveInfeedConditioningController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryCloveInfeedConditioningController", true); //ambil yang sudah submit saja
		...
		public ActionResult GetApproverList(long lphid)
		{
			try
			{
				...
				//		approvalList.ElementAt(0).Approver = GetFullName(approvalList.ElementAt(0).ApproverID);
				//	}
				...
				//                    approvalList.ElementAt(2).Role = "Requestor";
				//                }
				//                if (approvalList.Count == 4)
				//                {
				//                    approvalList.ElementAt(0).User = GetFullName(approvalList.ElementAt(0).UserID);
				//                    approvalList.ElementAt(0).Role = "Requestor";
				//                    approvalList.ElementAt(1).User = GetFullName(approvalList.ElementAt(1).UserID);
				...
				//                    approvalList.ElementAt(2).User = GetFullName(approvalList.ElementAt(2).UserID);
				//                    approvalList.ElementAt(2).Role = "Requestor";
				//                    approvalList.ElementAt(3).User = GetFullName(approvalList.ElementAt(3).UserID);
				//                    approvalList.ElementAt(3).Role = "Approver";
				//                }
				//            }
