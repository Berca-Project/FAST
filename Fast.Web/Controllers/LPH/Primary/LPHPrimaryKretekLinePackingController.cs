...
{
	[CustomAuthorize("primarykreteklinepacking")]
	public class LPHPrimaryKretekLinePackingController : BaseController<PPLPHModel>
	{
		...
		public LPHPrimaryKretekLinePackingController(
		...
		// GET: LPHPrimaryKretekLinePackingController
		public ActionResult Index(bool reset = false, bool restore = false)
		{
			...
			if (!reset)
			{
				...
				lphFilter.Add(new QueryFilter("MenuTitle", "LPHPrimaryKretekLinePackingController"));
		...
		public ActionResult getPSSData(String BatchID)
		{
			try
			{
				...
				var queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData] 
                    ...
                    (EventType = 'END Pack1 E' and DataName in ('ReportsiloUsed','LoadCartonSp')) or
                    (EventType = 'END Pack1 P' and DataName in ('BLEND')) or
                    (EventType = 'ST Pack1 E' and DataName in ('Batch ID')))";
				...
				if (AccountProdCenterID == 4 || AccountProdCenterID == 5)
				{
					...
					//" DataName in('Blend Code','SAP production Order',"+
					//" 'END_Packing_Size')"+
					//" ORDER BY EventType DESC";
					queryAle = @"SELECT [EventDateTimeLocal],[EventType],[DataName],[Value] FROM [NpssReportBatchData]
                        WHERE EventID = (SELECT [BatchID] FROM [NpssReportBatch] WHERE BatchIdent='" + BatchID + @"' and starttimelocal >= starttimelocal - 100)and(
                            (EventType ='ST Packing' and DataName in('Batch ID','Blend Code','SAP production Order'))
	                        or  (EventType ='END Packing' and DataName in('Batch ID'))
                       or DataName in('END_AB_CFSiloLoadSel','END_Packing_Bin_TobNetWeight','END_Packing_Bin_TobNetWeight','END_Packing_Bin_Qty'))";
		...
		// GET: LPHPrimaryKretekLinePackingController/Details/5
		public ActionResult Details(int id)
		{
			return View();
		}

		// GET: LPHPrimaryKretekLinePackingController/Create
		public ActionResult Create()
		{
			return View();
		}

		// POST: LPHPrimaryKretekLinePackingController/Create
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLinePackingController/Edit/5
		...
		// POST: LPHPrimaryKretekLinePackingController/Edit/5
		[HttpPost]
		...
		// GET: LPHPrimaryKretekLinePackingController/Delete/5
		public ActionResult Delete(int id)
		{
			return View();
		}

		// POST: LPHPrimaryKretekLinePackingController/Delete/5
		[HttpPost]
		...
		public ActionResult CheckPrev(long lphid = 0)
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _ppLphSubmissionsAppService.FindBy("LPHHeader", "LPHPrimaryKretekLinePackingController", true); //ambil yang sudah submit saja
