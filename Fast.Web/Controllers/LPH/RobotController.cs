...
{
	[CustomAuthorize("robot")]
	public class RobotController : BaseController<LPHModel>
	{
		#region ::Services::
		...
		public ActionResult Index(bool reset = false)
		{
			...
			LoadLPHHeader(model, _locationAppService, _employeeAppService, _userAppService, _userRoleAppService, _machineAppService, _brandAppService, _jobTitleAppService, _wppAppService, null, 8);
			...
			machineList = machineList.Where(x => x.Code.StartsWith("CP") && x.Code.Length > 1).OrderBy(x => x.Code).ToList();

			if (model.Machines.Count > 0)
			{
				machineList = machineList.Where(x => model.Machines.Any(z => z == x.Code)).OrderBy(x => x.Code).ToList();
			}

			machineList = machineList.Where(x => locationIdList.Any(y => y == x.LocationID)).ToList();
			ViewBag.RobotPalletizer = machineList.GroupBy(x => x.Code).Select(x => x.FirstOrDefault()).ToList();
		...
		public ActionResult Edit(long lphid)
		{
			...
			LoadLPHHeader(model2, _locationAppService, _employeeAppService, _userAppService, _userRoleAppService, _machineAppService, _brandAppService, _jobTitleAppService, _wppAppService, null, 8);
			...
			if (lphModel != null && lphModel.Header != null)
			{
				...
				if (model2.Machines.Count > 0)
				{
					machineList = machineList.Where(x => model2.Machines.Any(z => z == x.Code)).OrderBy(x => x.Code).ToList();
				}
		...
		public ActionResult Approval(long lphid)
		{
            ...
            LoadLPHHeaderApproval(model2, _locationAppService, _employeeAppService, _userAppService, _userRoleAppService, _machineAppService, _brandAppService, _jobTitleAppService, _wppAppService, null, 8, model.LPH.LocationID);
		...
		public ActionResult CheckPrev(long lphid = 0, string machine = "")
		{
			try
			{
				if (lphid != 0)
				{
					string submissions = _lphSubmissionsAppService.FindBy("LPHHeader", "RobotController", true); //ambil yang sudah submit saja
		...
		public ActionResult Submitted(long lphid, int type = 0)
		{
			...
			ViewBag.PageHeader = "LPH Robot";
			if (type == 1)
				ViewBag.PageHeader = "LPH Robot <span class='tx-success'>(Previous Submit)</span>";
			...
			LoadLPHHeader(model2, _locationAppService, _employeeAppService, _userAppService, _userRoleAppService, _machineAppService, _brandAppService, _jobTitleAppService, _wppAppService, null, 8, model.LPH.LocationID);
