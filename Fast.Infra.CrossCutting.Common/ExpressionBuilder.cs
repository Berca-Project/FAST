...
namespace Fast.Infra.CrossCutting.Common
{
    public class ExpressionBuilder
    {
            ...
            else
            {
                ...
                else
                {
                    while (filters.Count > 0)
                    {
                        ...
                        {
                            if (filters[0].Operation == Operation.AndAlso)
                                exp = Expression.AndAlso(exp, GetExpression<T>(param, filters[0], filters[1]));
                            else if (filters[0].Operation == Operation.Or)
                                exp = Expression.Or(exp, GetExpression<T>(param, filters[0], filters[1]));
                            else if (filters[0].Operation == Operation.And)
                                exp = Expression.And(exp, GetExpression<T>(param, filters[0], filters[1]));
                            else 
                        ...
                        if (filters.Count == 1)
                        {
                            if (filters[0].Operation == Operation.AndAlso)
                                ...
                                exp = Expression.And(exp, GetExpression<T>(param, filters[0]));
                            else
        ...
        private static Expression GetExpression<T>(ParameterExpression param, QueryFilter queryFilter)
        {
            ...
            switch (queryFilter.Operator)
            {
                case Operator.Equals:
                    if (constant == null)
                    {
                        var filter = Expression.Constant(Convert.ChangeType(queryFilter.Value, member.Type.GetGenericArguments()[0]));
                        Expression typeFilter = Expression.Convert(filter, member.Type);
                        return Expression.Equal(member, typeFilter);
                    }
                    else
                    {
        ...
        private static BinaryExpression GetExpression<T>(ParameterExpression param, QueryFilter filter1, QueryFilter filter2)
        {
            ...
            if (filter2.Operation == Operation.And)
                return Expression.And(result1, result2);
            else if (filter2.Operation == Operation.Or)
                return Expression.Or(result1, result2);
            else if (filter2.Operation == Operation.OrElse)
